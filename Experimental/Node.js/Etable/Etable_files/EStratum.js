...
do
 ...
 } while ((this.row[row].cell[col].type != "data") && (tries <= this.rows * this.cols)); 
...
//alert("this.emptycol="+this.emptycol)
     ...
     {
	  var entry1=entry;
	  if (opener && opener.checkcode) 
	      {entry=opener.checkcode(this.inputR,this.inputC,entry)}
	  else if(parent && parent.checkcode)
	      {entry=parent.checkcode(this.inputR,this.inputC,entry)}	  
