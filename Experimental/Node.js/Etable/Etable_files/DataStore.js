...
for(tr=rmin; tr<=rmax; tr++) 
 {
   ...
   {
     if (myTable.row[tr].cell[tc].type=="data")
...
for(r=0; r<myTable.rows; r++) 
 {
   ...
   for (c=0; c<myTable.cols; c++)
    {    
		 ...
		 t=myTable.row[r].cell[c].type;  //cell type
		 
		 if (t != "data" && t.length>0)
		 { 
		  if (metaData[t+"s"] != null)
		   { 
		     metaData[t+"s"]+=1;   //Increment count for this type
									//e.g.,  metadata["labels"]
		   }
		 }
		 metaData["R"+r][c]=v;
		 metaData["R"+r+"type"][c]=t;
		 if (t=="valname")
		   {
