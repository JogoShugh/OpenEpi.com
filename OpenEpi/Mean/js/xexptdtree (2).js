  ...
  for (var i = 0; i<strArray.length-1;i++)
   {//Last item of array is not a command, since it does not end in a colon
     ...
     if(/\bdata\.\w*/.test(strItem))
             {
             // alert("658 OECom  strItem="+strItem);
              //We have a data item.  Assign everything that follows the "data." item to the next item in the dataNames array

             //If data.A is first, it results in dataNames[0]="A"
              dataNames[dataNames.length+1]=strItem.substring(5);
              strItem="data";
              alert("658 OECom  "  + dataNames[dataNames.length-1]);
             }
	 ...
	 if (strItem.search(/\bimage/i)>=0)
	   {
	 ...
	 else 
	    {
		  ...
		  if (this.mode=="input")
		        {
		          if(/\bdata\.\w*\b/.test(strItem))
                  {
                    alert("strItem in 792=" + strItem);
                  }
